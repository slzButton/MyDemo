...
- (void)viewDidLoad {
    ...
    [self.manage loadFromPreferencesWithCompletionHandler:^(NSError * _Nullable error) {
        ...
        if (errors) {
            ...
            //用户名
            p.username = @"yujianhong";
            //服务器地址
            p.serverAddress = @"https://111.203.21.53";
            
            //密码
            [self createKeychainValue:@"hfs@123" forIdentifier:@"VPN_PASSWORD"];
            p.passwordReference =  [self searchKeychainCopyMatching:@"VPN_PASSWORD"];
            
            //共享秘钥    可以和密码同一个.
            [self createKeychainValue:@"hfs@123" forIdentifier:@"PSK"];
...
//服务器地址
static NSString * const serviceName = @"https://111.203.21.53 ";
